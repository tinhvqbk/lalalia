...
module.exports = function (fileName, converter) {
  if (!fileName) {
    throw new _pkg.PluginError('@hcjlab/core#data', 'Missing fileName option!');
  }    
  if (!converter) {
    throw new _pkg.PluginError('@hcjlab/core#data', 'Missing converter option!');
  }
  let data = {};
  let firstFile = null;
  ...
  function bufferContents(file) {
    ...
    if (file.isStream()) {
    skipConversion = true;
    return  this.emit('error', new _pkg.PluginError('@hcjlab/core#data', 'Streaming not supported'));
    }
    try {
      ...
      new _pkg.PluginError('@hcjlab/core#data', 'Error parsing JSON: ' + err + ', file: ' + file.path.slice(file.base.length)));
    }
  }
  function endStream() {
    ...
    this.emit('data', joinedFile);
        ...
        return this.emit('error', new _pkg.PluginError('@hcjlab/core#data', e, { showStack: true }));
      }
    }
    this.emit('end');
  }
