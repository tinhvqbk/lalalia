  ...
  function minify(file, _, cb) {
    ...
    try {
      ...
      if (!result || result.error) {
        const message = new _pkg.PluginError('lalalia#jsmin', 'uglify-es minify error.' + result.error)
      ...
      const message = new _pkg.PluginError('lalalia#jsmin', err, { fileName: file.path });
      process.stderr.write(`${message}\n`);
      _this.emit('end');
    }
  }
